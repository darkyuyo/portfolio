...
export default function AudioPlayer({ hidden }: { hidden?: boolean }) {
  ...
  return (
    <div style={{
      position: 'fixed',
      top: dockDesktop ? 'auto' : 'max(12px, env(safe-area-inset-top, 0px))',
      bottom: dockDesktop ? 'max(16px, env(safe-area-inset-bottom, 0px))' : 'auto',
      left: dockDesktop ? 'auto' : 'max(12px, env(safe-area-inset-left, 0px))',
      right: dockDesktop ? 'max(16px, env(safe-area-inset-right, 0px))' : 'auto',
      zIndex: 30,
      display: 'flex',
      alignItems: 'center',
      gap: '10px',
